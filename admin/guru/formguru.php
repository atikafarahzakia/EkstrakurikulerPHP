...
// Ambil daftar semua guru untuk pilihan dropdown
$gurus = mysqli_query($db, "SELECT id_guru, nama_guru FROM register_guru ORDER BY nama_guru");

// Ambil daftar semua ekskul untuk checkbox
$ekskuls = mysqli_query($db, "SELECT id_ekskul, nama_ekskul FROM tb_ekskul ORDER BY nama_ekskul");

if (isset($_POST['simpan'])) {

    $id_guru = (int)$_POST['id_guru'];             // Ambil ID guru yang dipilih
    $ekskul_ids = isset($_POST['ekskul_ids']) ? $_POST['ekskul_ids'] : []; // Ambil daftar ekskul yang dipilih

    // Validasi harus memilih guru dan ekskul minimal satu
    if ($id_guru > 0 && !empty($ekskul_ids)) {

        // Ambil nama guru berdasarkan id
        ...
        // Simpan ke tb_pembina menggunakan ekskul pertama
        ...
        // Ambil id pembina yang baru saja dimasukkan
        $id_pembina = mysqli_insert_id($db);

        // Insert semua ekskul ke tabel relasi
        foreach ($ekskul_ids as $ie) {
            $ie = (int)$ie;
            $res = mysqli_query($db, "
                INSERT INTO tb_ekskul_pembina (id_ekskul, id_guru)
                VALUES ($ie, $id_guru)
            ");
            if (!$res) {
        ...
        // Notifikasi berhasil dan redirect ke dashboard
        echo "<script>
...
<body>
    ...
    <div class="container bg-white p-4 mt-5 rounded shadow">
        ...
        <form action="" method="POST">
            ...
            <!-- Checkbox pilih ekskul lebih dari satu -->
            <label class="form-label">Pilih Ekskul (bisa lebih dari 1)</label>
            <div class="mb-3">
                <?php while ($e = mysqli_fetch_assoc($ekskuls)): ?>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="ekskul_ids[]"
                            value="<?= $e['id_ekskul'] ?>" id="ekskul-<?= $e['id_ekskul'] ?>">
                        <label class="form-check-label" for="ekskul-<?= $e['id_ekskul'] ?>">
