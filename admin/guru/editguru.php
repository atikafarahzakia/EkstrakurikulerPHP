...
// Pastikan URL membawa id_pembina agar halaman ini tahu data siapa yang di-edit
if (!isset($_GET['id_pembina'])) {
    die("ERROR: id_pembina tidak ditemukan di URL");
}

// Mengambil id_pembina dari URL dan memastikan tipe datanya integer
$id_pembina = (int)$_GET['id_pembina'];

// Mengambil data pembina berdasarkan id_pembina
...
// Ambil seluruh ekskul yang dipegang guru tersebut
$selectedEkskul = [];
$q = mysqli_query($db, "SELECT id_ekskul FROM tb_ekskul_pembina WHERE id_guru = {$guru['id_guru']}");
if ($q) {
    while ($r = mysqli_fetch_assoc($q)) {
        $selectedEkskul[] = $r['id_ekskul']; // memasukkan id ekskul ke dalam array
    }
}

// Proses jika tombol "ubah" ditekan (form disubmit)
if (isset($_POST['ubah'])) {
    $id_guru = (int)$_POST['id_guru'];  // ID guru
    $ekskul_ids = isset($_POST['ekskul_ids']) ? $_POST['ekskul_ids'] : []; // daftar ekskul yang dipilih
    ...
    // Hapus semua relasi ekskul-guru yang lama
    mysqli_query($db, "DELETE FROM tb_ekskul_pembina WHERE id_guru=$id_guru");

    // Menyimpan relasi baru ke tabel pivot tb_ekskul_pembina
    ...
    <form action="" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nama_pembina" class="form-control"
                   value="<?= htmlspecialchars($guru['nama_pembina']); ?>" required>
        </div>
        ...
        <div class="mb-3">
            ...
            if ($query) {
                while ($row = mysqli_fetch_assoc($query)) {
                    // Cek apakah ekskul ini dimiliki oleh guru tersebut
                    ...
                    echo "<div class='form-check'>
                            <input 
                                ...
                                name='ekskul_ids[]' 
                                value='{$row['id_ekskul']}' 
                                id='ekskul-{$row['id_ekskul']}' 
                                $checked
                            >
                            <label class='form-check-label' for='ekskul-{$row['id_ekskul']}'>
                                {$row['nama_ekskul']}
                            </label>
                          </div>";
                }
